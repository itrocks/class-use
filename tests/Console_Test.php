...
class Console_Test extends TestCase
{
	...
	public function testRunScan(int $index, array $arguments) : void
	{
		$model = new class extends Console {
			...
			protected function scan  ($arguments) : void { $this->call[__FUNCTION__] = true; }
			protected function search($arguments) : void { $this->call[__FUNCTION__] = true; }
		};
		$console = clone $model;
		$console->run($arguments);
		self::assertEquals(['scan' => true, 'search' => false], $console->call, $index);
	}
